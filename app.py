from apps import timer, translate, tik_tak_toe
import argparse
import asyncio

def main():
    parser = argparse.ArgumentParser(description="Selecciona qué aplicación iniciar")
    parser.add_argument("app", choices=["timer", "translate", "tictactoe"], help="Aplicación a ejecutar")
    ...
    elif args.app == "translate":
        start_translate(args.lang)
    elif args.app == "tictactoe":
        start_tictactoe(args.num)

def start_timer(cube_type):
    print(timer.generate_algorithm(cube_type))
...
def start_translate(lang):
    ...
    ### start_translate("English")
    "Hello"
    "Hola"
    """
    str = input("Insert your text to translate: ")
    return print(asyncio.run(translate.translate_text(lang, str)))
